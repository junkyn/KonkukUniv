...
public abstract class UserData implements Observable {
    ...
    private boolean checkPhoneFormat(String number) {
        if (number.matches("^[0-9-]*$")) {
            ...
            if (check.startsWith("010")) {
                if (check.length() == 11) {
                    phone =
                        check.substring(0, 3) + "-" + check.substring(3, 7) + "-" + check.substring(
                            7, 11);
                    return true;
                } else {
                    return false;
                ...
                if (check.length() == 9) {
                    phone =
                        check.substring(0, 2) + "-" + check.substring(2, 5) + "-" + check.substring(
                            5, 9);
                    return true;
                } else if (check.length() == 10) {
                    phone =
                        check.substring(0, 2) + "-" + check.substring(2, 6) + "-" + check.substring(
                            6, 10);
                    return true;
                } else {
                    return false;
                ...
                phone =
                    check.substring(0, 3) + "-" + check.substring(3, 6) + "-" + check.substring(6,
                        10);
                return true;
            } else if (check.length() == 11) {
                phone =
                    check.substring(0, 3) + "-" + check.substring(3, 7) + "-" + check.substring(7,
                        11);
                return true;
            } else {
                phone = check;
    ...
    public boolean validateBirthday(String birthday) {
        if (birthday.contains("\t")) {
            ...
            return isBirthdayPresent(birthday);
        }
    ...
    private boolean isBirthdayPresent(String birthday) {
        if (8 <= birthday.length() && birthday.length() <= 10 && birthday.matches(
            "^(19|20)\\d\\d[-./]?(0[1-9]|1[0-2]|[1-9])[-./]?(([0-2][1-9]|3[01]|[1-9])|(1[0-9]|2[0-8])|(29(?!-02-29|/02/29|\\.02\\.29)))$")) {
