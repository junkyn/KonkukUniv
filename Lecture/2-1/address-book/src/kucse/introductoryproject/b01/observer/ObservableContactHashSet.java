...
public class ObservableContactHashSet extends HashSet<Contact> implements Observable, Observer {
    ...
    public boolean isPhoneDuplicated(String phone) {
        return this.stream().anyMatch(it -> StringUtil.getNumbersOnly(it.getPhone())
            .equals(StringUtil.getNumbersOnly(phone)));
    }
    ...
    public String renameFrom(String fromName, String toName) {
        AtomicInteger count = new AtomicInteger();
        if (this.stream().filter(it -> !it.getName().equals(fromName))
            .anyMatch(it -> it.getName().equals(toName))) {
            count.incrementAndGet();
        }
